...
saveDeliveryRegionIdUrl : "/buyflowApp/server/order/saveDeliveryRegionId.jsx",
cartUrl : "/cart.html",
...
self.totalPrice = ko.observable(data.totalPrice);
self.unitDealPrice = ko.observable(data.unitDealPrice || data.unitPrice);
/*满减后的成交价*/
self.totalDealPrice = ko.observable(data.totalDealPrice || data.totalPrice);
/*满减后的商品总成交价*/
self.totalPrice = ko.observable(data.totalPrice);
...
self.canDelivery = data.canDelivery;
self.sellableCount = data.sellableCount;
...
if (self.deliveryResults().length == 1) {
...
self.mobileOk = function(item){
if(self.selectedRule().id != item.id){
...
self.regionPathName = ko.observable(data.regionPathName);
self.regions = $.map(data.options, function (region) {
return new Region(region);
});
self.currentRegionId = ko.observable(null);
self.currentRegionId(data.currentId);
self.currentRegionId.subscribe(function (newValue) {
if (self.onChange) {
self.onChange(self);
}
});
self.currentRegion = function () {
for (var i = 0; i < self.regions.length; i++) {
var region = self.regions[i];
if (region.id == self.currentRegionId()) {
return region;
}
}
return null;
};
self.currentRegionName = function () {
for (var i = 0; i < self.regions.length; i++) {
var region = self.regions[i];
if (region.id == self.currentRegionId()) {
return region.name;
...
self.currentRegionParentId = function () {
for (var i = 0; i < self.regions.length; i++) {
var region = self.regions[i];
if (region.id == self.currentRegionId()) {
...
self.currentRegionPathName = ko.observable();
self.regionLevels = ko.observableArray();

self.currentRegionLevel = ko.observable({});

self.setCurrentRegionLevel = function (regionLevel) {
self.currentRegionLevel(regionLevel);
};
self.setRegionLevels = function (levels) {
if (!levels) {
levels = [];
}
if (levels.length > 0) {
var last = levels[levels.length - 1];
if (!last.regions || last.regions.length == 0) {
levels.pop();
}
}
var regionLevels = $.map(levels, function (level) {
return new RegionLevel(level);
});
for (var i = 0; i < regionLevels.length; i++) {
var level = regionLevels[i];
var next = null;
if (i < regionLevels.length - 1) {
next = regionLevels[i + 1];
}
level.nextLevel = next;
level.levelIndex = i;
level.onChange = self.onLevelChange;
}
self.regionLevels(regionLevels);
self.currentRegionPathName(self.getCurrentRegionPathName());
self.setCurrentRegionLevel(regionLevels[regionLevels.length - 1]);
};
self.addRegionLevel = function (level) {
if (!level || !level.options || level.options.length == 0) {
return;
}
var regionLevel = new RegionLevel(level);
regionLevel.levelIndex = level.levelIndex;
var levels = self.regionLevels();
levels[level.levelIndex].currentRegionId(level.currentId);
//删除一些
levels.splice(level.levelIndex + 1, levels.length - level.levelIndex);
levels.push(regionLevel);
self.regionLevels(levels);
};
self.onNewLevel = function (newRegionLevel) {
newRegionLevel.levelIndex = self.regionLevels.length;
self.regionLevels.push(newRegionLevel);
};
self.getCurrentRegion = function () {
if (!self.regionLevels() || self.regionLevels().length == 0) {
return null;
}
var levels = self.regionLevels();
for (var i = levels.length - 1; i >= 0; i--) {
var level = levels[i];
if (level.currentRegionId()) {
return level.currentRegion();
}
}
};

self.getCurrentRegionId = function () {
if (!self.regionLevels() || self.regionLevels().length == 0) {
return null;
}
var levels = self.regionLevels();
...
self.getCurrentRegionPath = function () {
var path = [];
var levels = self.regionLevels();
for (var i = 0; i < levels.length; i++) {
var level = levels[i];
var currentRegion = level.currentRegion();
if (currentRegion) {
...
self.getCurrentRegionPathName = function () {
var path = self.getCurrentRegionPath();
var names = [];
$.each(path, function (index, elem) {
names.push(elem.name);
});
...
if (ret.state == 'ok') {
self.setRegionLevels(ret.regionLevels);
typeof(callback) == "function" ? callback() : null;
}
}, "JSON");
};
self.showRegionSelectorView = function () {
if (!$("#regionSelectView").is(":visible")) {
$("#regionSelectView").show();
}
};
self.hideRegionSelectorView = function () {
if ($("#regionSelectView").is(":visible")) {
$("#regionSelectView").hide();
}
};

self.loadRegionChildren = function (data) {
$.post(AppConfig.url + AppConfig.getRegionChildrenUrl, {regionId: data.parentId}, function (ret) {
if (ret.state == 'ok') {
self.setCurrentRegionLevel(data);
}
}, "JSON");
};
self.loadRegionChildren2 = function (data) {
$.post(AppConfig.url + AppConfig.getRegionChildrenUrl, {regionId: data.id}, function (ret) {
if (ret.state == 'ok') {
self.currentRegionLevel().currentRegionId(data.id);
var options = ret.regionLevel.options;
if (!options || options.length == 0) {
$.post(AppConfig.url + AppConfig.saveDeliveryRegionIdUrl, {deliveryRegionId: data.id}, function (ret) {
location.reload();
}, 'JSON');
} else {
var region = {
id: data.id,
options: options,
levelIndex: self.currentRegionLevel().levelIndex,
currentId: data.id
};
var regionLevel = new RegionLevel(region);
regionLevel.levelIndex = self.currentRegionLevel().levelIndex + 1;
self.addRegionLevel(region);
self.setCurrentRegionLevel(regionLevel);
}
}
}, "JSON");
};
...
function Cart(data) {
var self = this;
self.merchantName = ko.observable(data.merchantName);//商家名称
self.merchantId = ko.observable(data.merchantId);//商家ID
self.merchantMainColumnId = ko.observable(data.merchantMainColumnId);//商家主分类ID
...
self.isCollectTax = data.collectTax || false;/*是否收税*/
...
self.totalProductNumber = ko.computed(function(){
var total = 0;
$.each(self.buyItems(),function(value){
total += this.number();
});
$.each(self.orderRuleTargets,function(){
total += this.items&&this.items.length;
});
return total;
});
/*这个购物车是否需要交税*/
...
self.cartTotalCount = ko.observable();//购物车总数量
...
var number = 0;
$.each(self.carts(),function(idx,cart){
number += cart.totalProductNumber();
});
return number;
});
self.checkedTotalWeight = ko.computed(function(){
...
self.loadCarts = _.debounce(function(mode){

var filterPostData = {};
if(mode){
filterPostData.mode = mode;
}
$.post(AppConfig.url+"/appMarket/appEditor/filterCart.jsp",filterPostData,function(data){
if(data.code != "ok"){
return;
}
...
if(AppConfig.subSpec){
postData.subSpec = AppConfig.subSpec; //子商品图标大小 eg. 40X40
}
...
self.cartTotalCount(0);
$(document).trigger("cartsLoaded");
}
if(data.state=='ok'){
!isNaN(data.count)&&self.cartTotalCount(data.count);//获取当前频道购物车数量
...
var str = "";
$.each(data.mids,function(){
str += this +",";
});
}
self.mids(str);
self.openConfig = data.openConfig || {};
...
if(data.regions){
self.regionSelector.loadRegion(data.regionId);
}
}
},"json");
},"json");
...
if(self.overPriceLimit()){
confirmDialog.show("您的单笔订单金额超过海关限制:"+AppConfig.maxAllowCrossOrderPrice+"元，请合理选择商品进行结算!",null,"","",["ok"]);
return;
}
var note = self.checkOpenTime();
if(note){
confirmDialog.show("因春节假期影响，e万家网站的收单和派送将暂停一段时间。因此，您此单中所涉及的：<br>"+note,self.toOrderForm,"好，坚持下单","那，暂不下单","");
}else{
self.toOrderForm(isGift);
}
};

//去订单确认页
self.toOrderForm = function(isGift){
...
//检查商家配送的时间
self.checkOpenTime = function () {
if (!self.openConfig) {
return "";
}
var note = "";
$.each(self.carts(), function (idx, cart) {
if (cart.checkedNumber() > 0 && self.openConfig[cart.merchantMainColumnId()]) {
var config = self.openConfig[cart.merchantMainColumnId()];
var beginRestTime = config.beginRestTime;
var endRestTime = config.endRestTime;
if (beginRestTime) {
beginRestTime = Date.parse(beginRestTime);
}
if (endRestTime) {
endRestTime = Date.parse(endRestTime) + 24*60*60*1000;
}
if (self.currentTime > beginRestTime && self.currentTime < endRestTime) {
note += config.msg;
}
}
});
...
/*$.each(self.carts(), function (idx, cart) {
if (cart.isCrossBorder && cart.checkedNumber() > 0 && cart.isCollectTax) {
var totalPayPrice = cart.totalPayPrice();
if(Number(totalPayPrice) >= Number(AppConfig.maxAllowCrossOrderPrice)){
canCheckOut = false;
}
}
});*/
...
/*是否超出跨境金额限制*/
self.overPriceLimit = ko.computed(function(){
